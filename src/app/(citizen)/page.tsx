...
export default function CitizenPage() {
  ...
  if (!loading && !session) {
    return (
      <main className="min-h-screen bg-slate-50 flex items-center justify-center p-4 md:p-6 font-sans">
        <div className="w-full max-w-sm space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {authStage === 'phone' ? (
              ...
              <div className="space-y-6">
                <div className="space-y-2 text-center">
                  ...
                  />
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest pt-2 px-2">Verification code sent to {phoneNumber}</p>
                </div>
                <Button onClick={handleVerify} disabled={authLoading || otp.length < 6} className="w-full h-12 md:h-14 bg-slate-900 text-white rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-[0.2em] shadow-lg shadow-slate-200">
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-100 via-slate-50 to-white py-10 md:py-16 px-4 animate-in fade-in duration-300">
      <div className="max-w-xl mx-auto space-y-8 md:space-y-10">
        <header className="space-y-4 text-center">
          <div className="flex justify-between items-center px-3 md:px-4 py-2 bg-white rounded-full mb-6 md:mb-8 shadow-sm border border-slate-100">
            <div className="flex items-center gap-1.5 md:gap-2">
              <div className="w-1.5 h-1.5 md:w-2 md:h-2 bg-green-500 rounded-full" />
              <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-slate-500">Verified Citizen</span>
            </div>
            <button onClick={() => { authProvider.logout(); window.location.reload(); }} className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-red-500 transition-colors">Logout</button>
          </div>
          <div className="space-y-1 md:space-y-2">
            <h1 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight uppercase italic leading-none">
              CIVIC <span className="text-slate-500">SENTINEL</span>
            </h1>
            <p className="text-sm md:text-lg text-slate-500 font-medium tracking-tight px-4">
          ...
          <div className="space-y-4">
            {fetchingGrievances ? (
              ...
              <div className="p-8 md:p-10 bg-white rounded-3xl md:rounded-[2.5rem] border border-slate-100 shadow-sm text-center space-y-4 group overflow-hidden relative transition-all duration-500 hover:shadow-xl hover:shadow-slate-200/50">
                ...
                <div className="mx-auto w-10 md:w-12 h-10 md:h-12 bg-slate-50 rounded-xl md:rounded-2xl flex items-center justify-center group-hover:bg-slate-900 transition-colors duration-500">
                  <Clock className="w-4 md:w-5 h-4 md:h-5 text-slate-300 group-hover:text-white transition-colors" />
                </div>
                <div className="space-y-1 relative z-10">
                  <h3 className="text-xs md:text-sm font-black uppercase tracking-tight text-slate-900">No active oversight</h3>
                  <p className="text-[10px] md:text-xs text-slate-400 font-medium max-w-[200px] mx-auto leading-relaxed">Your reported issues will appear here with an active countdown.</p>
                </div>
              </div>
            ) : (
              myGrievances.map((g) => {
                  ...
                  >
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-4 relative z-10">
                      <div className="space-y-1.5 flex-1 pr-0 sm:pr-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[9px] md:text-[10px] text-slate-300 font-medium font-mono">{g.id.split('-')[0]}</span>
                        </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-50 group-hover:border-slate-100 transition-colors">
                      <div className="flex items-center gap-3 md:gap-4">
                        {g.imageUrl && (
                          <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl overflow-hidden shadow-sm border border-slate-100 ring-4 ring-slate-50 transition-all group-hover:scale-110 group-hover:ring-slate-100 duration-500">
                        ...
                        <div className="space-y-0.5">
                          <div className="flex items-center gap-1 text-slate-400">
                            ...
                            <span className="text-[8px] md:text-[10px] font-bold uppercase tracking-tight text-slate-500 truncate max-w-[120px] sm:max-w-none">{g.location?.city || "Unknown Location"}</span>
                          </div>
                          <p className="text-[8px] md:text-[10px] text-slate-400 font-medium">{new Date(g.createdAt).toLocaleDateString()}</p>
                        </div>
                      </div>
                      <Button variant="ghost" size="icon" className="w-8 h-8 md:w-10 md:h-10 rounded-full hover:bg-slate-900 hover:text-white text-slate-300 transition-all duration-500">
